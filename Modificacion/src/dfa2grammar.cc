...
int main(int argc, char** argv) {
  if (argc == 3) {
    ...
    if (!std::regex_match(automata_file, automata__file_regex)) {
      std::cout << std::endl;
      std::cout << "ERROR: El fichero de entrada de especificación del "
                   "autómata, no tiene el formato de fichero correcto."
                << std::endl;
    ...
    if (!std::regex_match(grammar_file, grammar_file_regex)) {
      std::cout << std::endl;
      std::cout << "ERROR: El fichero de salida de la gramática generada no "
                   "tiene el formato de fichero correcto."
                << std::endl;
      std::cout << "Para más información haga uso de la opción -h o --help"
                << std::endl;
      std::cout << "./dfa2grammar (-h | --help)" << std::endl;
    ...
    if (new_automata.IsDFA()) {
      ...
      // std::cout << "ERROR >>> No se pudo generar la gramática, debido a que el "
      //              "automata introducido no es un DFA."
      //           << std::endl;
      // std::cout << "Para la correcta ejecución del programa introduzca un DFA."
      //           << std::endl;
      // std::cout << "Para más información haga uso de la opción -h o --help"
      //           << std::endl;
      // std::cout << std::endl;
      // return 2;  /// Exit code 2
      new_automata.DFAConvertor();
      new_automata.PrintAutomata();
      Grammar new_grammar;
      new_grammar = new_automata.ConvertToGrammar(new_automata);
      new_grammar.PrintGrammar();
