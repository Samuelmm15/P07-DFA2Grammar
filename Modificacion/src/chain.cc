 ...
 * @brief This file contains the different methods of the chain class.
 * @version 0.1
 * @date 2022-11-10
 ...
 *
 */
Chain::~Chain() { chain_.clear(); };

/**
 * @brief This method adds the different introduced chains by the file into a
 * vector of chains.
 *
...
void Chain::AddChain(std::string newChain, Alphabet alphabet) {
  if (alphabet.AlphabetComprobation(newChain) == true) {
    ...
    std::cout << "La cadena " << newChain
              << " no pertenece al alfabeto introducido" << std::endl;
  }
  chain_ = newChain;
};
 ...
 */
std::string Chain::getChain() { return chain_; };